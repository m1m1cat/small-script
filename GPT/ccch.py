...
num_threads = int(input("请输入线程数（看网络能力）："))
...
def crawl_url(urls, lock):
    with lock:
        with open(os.path.join(result_dir, "jieguo.txt"), "a", encoding="utf-8") as f:
            for url in urls:
                try:
                    logging.info("开始抓取网站:%s", url)
                    res = requests.get(url, headers=headers, verify=True, proxies=proxies_dict)
                    logging.debug("请求内容: %s", res.request.headers)
                    logging.debug("相应内容: %s", res.headers)
                    ...
                    logger.error("SSL错误: %s", e, exc_info=True)
                    continue
                except RequestException as e:
                    logger.error("请求错误: %s", e, exc_info=True)
                    continue
                else:
                    logger.info("成功抓取网站 %s", url)
